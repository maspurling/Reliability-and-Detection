# -*- coding: utf-8 -*-
"""
    Time-interval tests for detecting anomalies
    dynamically as IVLs are given
    
    Returns worker correctness-confidence graphs with 
    anomalies highlighted as IVLs are introduced
...
def regLabels(k, w, a):
    ...
    '''
    if a: # Attackers
        r = list(np.random.gamma(.4, .12, len(k)))
        #r = [rnd.uniform(0, 1) for i in range(len(k))]
        low = [w[0] - r[i] for i in range(len(k))]
        up = [w[0] + r[i] for i in range(len(k))]
    else:
        r = [rnd.uniform(0, .02) for i in range(len(k))]
        low = [w[0] - r[i] for i in range(len(k))]
        up = [w[0] + r[i] for i in range(len(k))]
    for i in range(len(k)):
        if low[i] > up[i]:
...
def main(k=100):
    ...
    timeInt = [5, 10, 50, 100, 150, 200]
    anomX, anomY, normX, normY, eyes = [], [], [], [], []
    
    # Append normal workers for graphing purposes
    for i in range(k):
    ...
    regWorkers = [0] * k
    q = 0
    # Run workers on questions pools
    for h in timeInt:
        # Gold workers
        for i in range(k):
            gLow, gUp = goldLabels(goldQ[q:h], workers[i])
            w = Worker(gLow, gUp)
            goldWorkers[i] = ([w.mean, w.var])
        ...
        for i in range(k):
            if i in ans:
                ...
                a = False
            rLow, rUp = regLabels(regQ[q:h], workers[i], a)
            w = Worker(rLow, rUp)
            regWorkers[i] = ([w.mean, w.var])
        q = h

        # Run t-test and report anomalies
        tEyes = []
        for i in range(k):
            # Test Statistic
        ...
        # Run F-test and report anomalies
        for i in range(k):
            # Test Statistic
        ...
        plt.title(f"{h} Intervals", fontsize=15)
