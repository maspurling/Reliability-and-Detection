...
"""
    ...
    F-tests over an average
    
    Gives efficiency of detection methods for anomalous workers
...
def main(k=100):
    ...
    workersList = workers.values.tolist()
    k = len(workersList)
    runs = []
    for i in range(40):
        ...
        for i in range(k):
            gLow, gUp = goldLabels(list(goldQ), workersList[i])
            w = Worker(gLow, gUp)
        ...
        for i in range(k):
            if i in ans:
                ...
                a = False
            rLow, rUp = regLabels(list(regQ), workersList[i], a)
            w = Worker(rLow, rUp)
        ...
        runs.append(len(eyes)/len(ans))
    
    print(round(np.mean(runs) * 100, 1),'%', sep="")
    print(round(np.max(runs) * 100, 1), '%', sep="")
    print(round(np.min(runs) * 100, 1), '%', sep="")
