# -*- coding: utf-8 -*-
"""
    Worker correctness detection with 60% confidence threshold
    on interval-valued workers using t- and F-tests
    
    Returns anomalous workers and correctness-confidence
    figure with anomalies highlighted
...
def regLabels(k, w, a):
    ...
    '''
    if a: # Attackers
        r = list(np.random.gamma(.5, .1, len(k)))
        # r = [rnd.uniform(0, .106) for i in range(len(k))]
        low = [w[0] - r[i] for i in range(len(k))]
        up = [w[0] + r[i] for i in range(len(k))]
    else:
...
def main(k=100):
    ...
    results.to_excel('attackerSick.xlsx')'''
    ...
    plt.scatter(anomX, anomY, color='#661D98', marker='x', s=60)
    plt.title("Sick", fontsize=15)
    plt.xlabel("Correctness", fontsize=15)
    plt.ylabel("Confidence", fontsize=15)
    plt.show()
    
    wrkAtks.sort()
    print(f"Generated Attackers:\n{wrkAtks}\n")
    print(f"Found Attackers:\n{atks}")
